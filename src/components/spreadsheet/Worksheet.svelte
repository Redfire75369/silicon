<script lang="ts">
	...
	$: frozenX = worksheet.pane?.horizontal_split ?? 0;
	$: frozenY = worksheet.pane?.vertical_split ?? 0;

	$: rows = worksheet.rows.map((row, r) => {
		return row.cells.map((cell, c) => {
			const sticky = (r < frozenY || c < frozenX) ? "cell sticky" : "cell";
...
<div class="scrollable">
	<div class="container" style="width: 100vw; height: calc(100vh - 40px);">
		<table class="table" style="width: {worksheet.column_widths.reduce((a, w) => a + w)}px; height: {worksheet.row_heights.reduce((a, h) => a + h)}px;">
			<colgroup>
				...
				{#each rows.slice(0, frozenY) as row}
					<tr>
						{#each row as {primary, column_span, row_span, style, sticky, cell}}
							{#if primary}
								<th scope="col" class={sticky} {style} colspan={column_span} rowspan={row_span}>
									<Cell {cell}/>
								</th>
							{:else}
			...
			<tbody>
				{#each rows.slice(frozenY, worksheet.rows.length) as row}
					<tr>
						{#each row as {primary, column_span, row_span, style, sticky, cell}}
							{#if primary}
								<td class={sticky} {style} colspan={column_span} rowspan={row_span}>
									<Cell {cell}/>
								</td>
							{:else}
